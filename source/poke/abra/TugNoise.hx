...
class TugNoise implements IFlxDestroyable
{
	...
	public function update(elapsed:Float)
	{
		...
		if (_tugNoiseTimer <= 0)
		{
			...
			if (tug)
			{
				SoundStackingFix.play(FlxG.random.getObject([AssetPaths.stretch0_00a3__mp3, AssetPaths.stretch1_00a3__mp3, AssetPaths.stretch2_00a3__mp3]), Math.sin(_tugNoiseSine * Math.PI) * 0.2 + 0.5);
				_tugNoiseTimer = 0.08;
				if (tugging)
				{
